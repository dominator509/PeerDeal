...
| `LEGACY-GAP-2026-06-13-007` | `docs/PRODUCTION_READINESS.md` | 4 | T4 | DEFERRED | Remaining native work is other-platform capture, local-network discovery, live transport, durable platform persistence, runtime key/capture validation, and non-Windows platform storage; JSON recovery wipe and app retention-coordinator seams are implemented, but production session-close scheduling and database/platform persistence remain open. |
