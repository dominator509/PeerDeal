...
| `LEGACY-GAP-2026-06-13-007` | `docs/PRODUCTION_READINESS.md` | 4 | T4 | DEFERRED | Remaining native work is other-platform capture, local-network discovery, live peer transport, durable platform persistence, runtime key/capture validation, and non-Windows platform storage; JSON recovery wipe, app retention coordination, exactly-once per-session close seams, protocol-event adapters, mirrored app session-runtime owners, canonical event-byte decoding, app frame-to-runtime handlers, and the bounded app transport source scheduler are implemented. Native peer transport implementation, platform source mounting, and database/platform persistence remain open. |
