...
| `LEGACY-GAP-2026-06-13-007` | `docs/PRODUCTION_READINESS.md` | 4 | T4 | DEFERRED | Remaining native work is other-platform capture, local-network discovery, live peer transport, durable platform persistence, runtime key/capture validation, and non-Windows platform storage; JSON recovery wipe, app retention coordination, exactly-once per-session close seams, protocol-event adapters, mirrored app session-runtime owners, canonical event-byte decoding, and app frame-to-runtime handlers are implemented. Native transport implementation/source mounting and database/platform persistence remain open. |
