...
The network transport validator and replay guard also reject values above the
shared signed 32-bit ceiling (`0x7fffffff`), matching the native bridge and
Android/Windows host envelope contract.
...
completion, caller cancellation, and the configured timeout. It passes each
source load an app-owned cancellation signal and completes that signal when
...
`BasicConflictDetector` and `BasicSnapshotApplier` reject direct caller-provided
event lists above `RecoveryEventWindowLimits.defaultMaxEvents` (4,096 by
...
assigning route class and priority. It also fails closed when the request's
session or table scope identity is malformed before assigning candidates.
Session path selection must ignore
...
- Generic native secure key storage method-channel requests must reject blank,
  padded, oversized, or control-bearing UTF-8 namespaces, key ids, and key
  record fields before platform calls; secure-key namespaces are capped at 128
  UTF-8 bytes and all other bounded text fields use limits consistently at the
  Dart and Android/Windows host boundaries.
