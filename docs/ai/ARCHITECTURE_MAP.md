...
| Native seams | `peerdeal_native_bridges` plus app hosts | Method-channel contracts for platform facts, generic byte transport, and secure key records; secure-key, local-network, transport, and app-storage calls have bounded default deadlines, with additive caller cancellation for app-owned lifecycle ownership; mobile Android and Windows desktop supply secure-key, capture, app-storage, local-network interface capability, and bounded multicast transport host implementations; Windows secure-key operations serialize per namespace across PeerDeal processes with a bounded Local named mutex |
