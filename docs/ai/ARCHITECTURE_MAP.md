...
| Privacy/receipt/capture | `peerdeal_privacy`, `peerdeal_receipts`, `peerdeal_capture` | Retention, receipt artifacts, capture policy; `DefaultDiagnosticsScrubber` bounds recursive map/list materialization, depth, text, and protocol diagnostic collections before app rendering; `OpaqueExportDecoder` bounds decoded artifact/payload JSON structure before receipt shape inspection; receipt-owned key-ring providers bound retained verification/decryption lookup collections before traversal |
