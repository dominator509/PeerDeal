...
| --- | --- | --- |
| App orchestration | `apps/peerdeal_mobile`, `apps/peerdeal_desktop` | Routes, setup/join flows, demo slices, app-owned presenters/controllers, protocol SessionClosed-to-retention mapping, recovery retention and exactly-once session-close coordination, app-owned table-session runtime projection, native-to-package mapping, native transport composition, native readiness aggregation; join role permissions, bootstrap peer candidates, outcome diagnostics, runtime route/mode/readiness collections, and production route/navigation extensions are snapshotted at the app boundary |
