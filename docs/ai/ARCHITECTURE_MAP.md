...
| --- | --- | --- |
| App orchestration | `apps/peerdeal_mobile`, `apps/peerdeal_desktop` | Routes, setup/join flows, demo slices, app-owned presenters/controllers, protocol SessionClosed-to-retention mapping, recovery retention and exactly-once session-close coordination, app-owned table-session runtime projection, native-to-package mapping, native transport composition, native readiness aggregation; join role permissions, bootstrap peer candidates, outcome diagnostics, runtime route/mode/readiness collections, production route/navigation extensions, and mounted table recovery warning results are snapshotted at the app boundary |
| Shared UI | `peerdeal_ui_kit` | Safe-surface widgets and render models; safe-surface warning/native-note collections and scaffold action collections are snapshotted at the shared UI boundary |
| Native seams | `peerdeal_native_bridges` plus app hosts | Method-channel contracts for platform facts, generic byte transport, and secure key records; generic Dart decoders and Android/Windows hosts enforce bounded UTF-8 secure-key fields, platform collections, frame payloads, identities, discovery values, app-storage paths, capture diagnostics, and other diagnostics before model construction or native persistence; Android and Windows local-network and transport hosts also bound interface/adapter enumeration and address buffers before channel serialization or multicast socket selection; secure-key, local-network, transport, and app-storage calls have bounded default deadlines, with additive caller cancellation for app-owned lifecycle ownership; mobile Android and Windows desktop supply secure-key, capture, app-storage, local-network interface capability, and bounded multicast transport host implementations; Android secure-key operations use encrypted private per-namespace files plus bounded private file locking and Windows uses a bounded Local named mutex for PeerDeal process serialization; both hosts persist namespace revisions and expose additive conditional mutations for stale-writer detection; the Windows desktop has a direct-run runtime smoke target for these channel registrations |
| Network confidence | `peerdeal_network` | Route class, bounded bootstrap/path/election collections, reserved-aware operational peer-id gates, confidence, primary peer selection, transport frame send/receive gates |
...
| Privacy/receipt/capture | `peerdeal_privacy`, `peerdeal_receipts`, `peerdeal_capture` | Retention, receipt artifacts, capture policy; `DefaultDiagnosticsScrubber` bounds recursive map/list materialization, depth, text, and protocol diagnostic collections before app rendering; `OpaqueExportDecoder` bounds decoded artifact/payload JSON structure before receipt shape inspection; receipt-owned artifact/inspection models freeze caller-owned maps and diagnostics, while key-ring providers freeze and bound retained verification/decryption lookup collections before traversal |
| Mode/variant policy | `peerdeal_modes`, `peerdeal_variants` | Session mode policy and poker variant rules; mode governance bounds participant, seat, and waitlist collections before traversal and freezes governance input/output collections at the package boundary; public variant result/state models defensively copy and freeze collections at the package boundary |
| Deterministic truth | `peerdeal_core` | Table state, reducer, invariants, pot/settlement primitives; core pot settlement bounds commitments and winner collections before traversal, `TableState` recursively freezes metadata, pot settlement models own public collections at construction, configurable reducer guards are snapshotted through `CoreReducer.withInvariantGuards`, and `TableState.fromJson` bounds materialized JSON through protocol canonical limits before typed hydration |
| Protocol | `peerdeal_protocol` | Envelopes, catalog, fixtures, diagnostics, bounded deterministic canonical JSON; command/event/snapshot payload trees are recursively frozen at construction, custom catalog entries and lock-report errors are owned at the boundary, and direct event/snapshot hydration validates full materialized trees |
...
1. App/session code supplies validated `HoldemHandState` and `HoldemEventCursor`
   to `AppHoldemTableSessionRuntime`; typed Hold'em state and snapshot hydration
   bounds materialized JSON through protocol canonical limits first.
...
3. Sync bounds direct caller-provided event lists before conflict detection or
   ...
   applies snapshot plus suffix events.
...
| Wizard/setup | `packages/peerdeal_wizard/lib/peerdeal_wizard.dart` (bounded preset/setup materialization and direct Game File plan validation through wizard-owned limits plus protocol canonical JSON; public setup, preset, validation, draft, and compile-result models freeze caller-owned collections) |
...
- Secure key storage has Dart/method-channel read/write seams plus app-owned
  ...
  versioned envelope in Credential Manager. Both hosts persist nonnegative
  namespace revisions and implement conditional save/delete conflict checks;
  runtime persistence validation and other platform implementations remain
...
`RoleGrant` permissions, `BootstrapPlan` peer candidates, and
